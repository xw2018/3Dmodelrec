...
def process_results(name: str, eval_model,
    ...
    eval_model.save("tem_model//eval_model.hdf5")


    #去掉全链接层
    # latent_model = _make_latent_model(eval_model)
    latent_model = eval_model
    ...
    modeldir,dir_path = initialize_results_dir(name, accuracy, mean_avg_prec)
    ...
    from myreload import mreload
    mreload(modeldir)
    print("\naccuracy=",accuracy)
    print("\nrot_accuarcy=",rot_accuracy)
    # not worth the time
    # # save tsne plots
    # print('\n\n\n##### Saving TSNE Recall #####\n\n\n')
