...
def L2(vectors,axis=-1):
    s_squared_norm = K.sum(K.square(vectors), axis, keepdims=True)
    result=K.sqrt(s_squared_norm + K.epsilon())
    return result
    ...
    def call(self, inputs, training=None):
        ...
        #DRDL
            if i < self.routings - 1:
                ...
                # outputs.shape=[None, num_capsule, dim_capsule] # [None, 10, 16]
                v = K.batch_dot(c, inputs_hat, [2, 2])
                b = K.batch_dot(v / L2(v), inputs_hat / L2(inputs_hat), [2, 3])
            ...
            if i == self.routings - 1:
                ...
                # d=abs(L2(v)-L2(inputs_hat))   #dims not equal,can not operate
