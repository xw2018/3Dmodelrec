...
if 'linux' in sys.platform:
    ...
    CMD = r"./binvox -cb -pb -e -c -d {dim} {path}"
elif 'darwin' in sys.platform:
    CMD = r'./binvox -cb -e -c -d {dim} {path}'
else:
...
def convert_off_to_binvox(path, dim=30):
    ...
    tempath=CMD.format(dim=dim, path=path).split()
    print(tempath)
    process = subprocess.Popen(tempath, stdout=subprocess.PIPE)
